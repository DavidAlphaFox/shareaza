...
components_plugins=Agregats (plug-in)
components_skins=Pells (skins)
tasks_languages=Idiomes
tasks_allusers=Tots els usuaris
tasks_selectusers=Instal·lar %1 per a:
tasks_currentuser=%1 únicament
tasks_multisetup=Habilita el suport multi-usuari
tasks_firewall=Afegeix una excepció al tallafocs del Windows
tasks_upnp=Habilita el descobriment de dispositius UPnP
tasks_deleteoldsetup=Esborra instal·lacions antigues
tasks_resetdiscoveryhostcache=Reestableix els serveis de descobriment i caxè de servidors
run_skinexe=Executant la instal·lació de la pell (skin)...
reg_incomingchat=Missatge de xat entrant
reg_apptitle=Shareaza: compartició d'arxius d'última generació
icons_license=Llicència
icons_uninstall=Desinstal·lació
icons_downloads=Descàrregues
icons_basicmode=Mode normal
icons_tabbedmode=Mode en pestanyes
icons_windowedmode=Mode de finestres
dialog_shutdown=%1 està sent executat. Dessitja que %1 siga aturat per que la instal·lació puga continuar?
dialog_firewall=La instal·lació ha fallat mentre s'afegia una exepció al tallafocs del Windows.%nSi us plau, afegeix Shareaza al llistat d'excepcions manualment.
dialog_malwaredetected=Shareaza ha detectat un arxiu maliciós (malware) a %1. Si us plau, elimine'l amb un antivirus abans d'instal·lar. Vol sortir de la instal·lació?
page_viruswarning_text=Mentre utilitzes Internet, has d'assegurar-te que tens un antivirus actualitzat per a protegir-te de troians, cucs, virus i altres programes maliciosos. Pots consultar un llistat de programari antivirus i consells de seguretat fent clic a la següent drecera:
page_viruswarning_title=Advertència de virus
page_viruswarning_subtitle=Tens un programa antivíric instal·lat?
PathNotExist=Error: la carpeta %1 no existeix.
