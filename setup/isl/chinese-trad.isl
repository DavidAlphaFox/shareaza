...
components_plugins=外掛
components_skins=外觀
tasks_languages=語言
tasks_allusers=所有的使用者
tasks_selectusers=為這些使用者安裝 %1:
tasks_currentuser=只有 %1
tasks_multisetup=啟用多重使用者支援
tasks_firewall=增加例外至 Windows 防火牆 (XP)
tasks_upnp=Enable discovery of UPnP devices
tasks_deleteoldsetup=刪除舊的安裝程式
tasks_resetdiscoveryhostcache=Reset Discovery and Hostcache
run_skinexe=正在安裝外觀...
reg_incomingchat=輸入的聊天訊息
reg_apptitle=Shareaza---最終的檔案分享軟體
icons_license=軟體授權協定
icons_uninstall=解除安裝
icons_downloads=下載
icons_basicmode=標準模式
icons_tabbedmode=標籤模式
icons_windowedmode=多重視窗模式
dialog_shutdown=%1 正在運作中. 您要關閉 %1 , 讓安裝程式得以繼續進行嗎?
dialog_firewall=安裝程式無法新增 Shareaza 至Windows 防火牆.%n請手動新增 Shareaza 至例外清單
...
page_viruswarning_text=使用網際網路時, 您應該總是確保防毒軟體為最新版本, 如此才能保護您避免受到木馬, 蠕蟲, 或是惡意程式的侵害. 您可以點選這個連結取得安全性秘訣與良好防毒軟體的清單:
page_viruswarning_title=病毒警告
page_viruswarning_subtitle=您是否已經安裝了一個防毒軟體?
