...
components_plugins=Plugin
components_skins=Skin
tasks_languages=Multi-lingua
tasks_allusers=Tutti gli utenti
tasks_selectusers=Installa %1 per:
tasks_currentuser=Solo %1
tasks_multisetup=Abilita il supporto multi-utente
tasks_firewall=Aggiungi un'eccezione a Windows Firewall
tasks_upnp=Abilita il rilevamento dei dispositivi UPnP
tasks_deleteoldsetup=Cancella installer vecchi
tasks_resetdiscoveryhostcache=Resetta i servizi di connessione e la cache host
run_skinexe=Installazione delle skin in corso...
reg_incomingchat=Messaggio di chat in arrivo
reg_apptitle=Shareaza, il programma definitivo di P2P
icons_license=Licenza
icons_uninstall=Disinstalla
icons_downloads=Download
icons_basicmode=Modalità normale
icons_tabbedmode=Modalità a tabelle
icons_windowedmode=Modalità a finestre
dialog_shutdown=%1 è attualmente in esecuzione. Vuoi che %1 sia chiuso così l'installazione può continuare?
dialog_firewall=Impossibile aggiungere Shareaza a Windows Firewall.%nAggiungi Shareaza alla lista delle eccezioni manualmente.
dialog_malwaredetected=Un programma malevolo (malware) è stato rilevato sul tuo sistema in %1, rimuovilo con un programma anti-virus/anti-malware prima di installare Shareaza. Vuoi uscire adesso?
page_viruswarning_text=Quando usi internet, dovresti sempre assicurarti di aver un antivirus aggiornato per proteggerti dai trojan, worm e dagli altri programmi malevoli. Puoi trovare una lista di buoni antivirus e altri suggerimenti di sicurezza per proteggere il tuo computer seguendo questo link:
page_viruswarning_title=Attenzione ai virus
page_viruswarning_subtitle=Hai installato un programma antivirus?
PathNotExist=Errore, il percorso della cartella %1 non esiste
